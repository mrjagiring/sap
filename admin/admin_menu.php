...
if($_POST[submit2]=="submit"){
	$root2=mysqli_result1(mysqli_query($mysqli1,"select `root` from `menu` where `name`='".$_POST[menuroot]."'"),0);
	mysqli_query($mysqli1,"INSERT INTO `menu` (`name`,`root`,`root2`, `priority`) VALUES ('".$_POST[menuname]."','".$_POST[menuroot]."','$root2','".$_POST[menupriority]."')");
	echo"Success Input New Menu";
	die();
}
if($_GET[delete]){
...
echo"<form method=post><table align=center style=text-align:center;width:600px; border=1><tr style=font-weight:bold;font-size:14px;><td colspan=3>Add Sub Menu 1</td></tr>";
...
echo"<tr style=font-weight:bold;font-size:12px;><td style=width:45%;>Root</td><td style=width:10%;> : </td><td style=width:45%;><select style=width:100%; name=menuroot>".listroot()."</select></td></tr>";
...
echo"<table align=center style=text-align:center;width:600px; border=1><tr><td colspan=3>List Menu Root</td></tr>";
...
while($ARRAY=mysqli_fetch_array($DATA)){
	echo"<tr style=font-size:12px;><td style=width:50%;>$ARRAY[name]</td><td style=width:50%;>[ <a href=editmenu.php?menuid=$ARRAY[id]>Edit</a> ] | [ <a onclick=\"window.open('rename_menu.php?type=1&id=$ARRAY[id]','mywin','left=200,top=200,width=400,height=150,toolbar=0,resizable=0');\"
 style=color:blue;cursor:hand;text-decoration:underline;>Rename</a> ]</td></tr>";
}
echo"</table>";
echo"<br>";
echo"<table align=center style=text-align:center;width:600px; border=1><tr><td colspan=3>List Menu Sub 1</td></tr>";
...
$DATA=mysqli_query($mysqli1,"select * from `menu` where `root`!='' order by`root`,`priority` asc");
while($ARRAY=mysqli_fetch_array($DATA)){
